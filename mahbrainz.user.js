 ...
 * Special thanks to Zanz and Babble Rabble for sharing their valuable knowledge on Zamgrh.
...
function brn_rrr_renderBrain(panel){
    ...
    groupedArray.forEach(group => {
        ...
        stamp.innerHTML =
            `${group.timeLocal}<br><span style="opacity:.6">${group.location}</span>`;
        ...
        single.onclick = () => brn_exportSingleMemry(group.items[0]);
...
function brn_exportSingleMemry(m){
    ...
    const output =
`[2;31m🧠 From the rotting brainz of:[0m[2;37m ${PLAYER_NAME}[0m
[2;37m══════════════════════════════[0m
...
localStorage.setItem("wwdead_memryz_pos", JSON.stringify({
